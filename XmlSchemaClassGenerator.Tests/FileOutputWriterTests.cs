...
public class FileOutputWriterTests(ITestOutputHelper output)
{
    ...
    public void TestDefaultProviderGeneratorPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "DefaultProviderGeneratorPrefix", PrefixPattern);
    ...
    public void TestEmptyKeyProvider()
    {
        ...
        SharedTestFunctions.TestSamples(output, "EmptyKeyProvider", PrefixPattern);
        Assert.Single(outputWriter.Files);
    ...
    public void TestEmptyKeyProviderGeneratorPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "EmptyKeyProviderGeneratorPrefix", PrefixPattern);
        Assert.Single(outputWriter.Files);
        Assert.Equal(Path.Combine(directory, "NamedNamespace.cs"), outputWriter.Files.First());
    }
    ...
    public void TestEmptyKeyProviderGeneratorConfigurationPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "EmptyKeyProviderGeneratorConfigurationPrefix", PrefixPattern);
        Assert.Single(outputWriter.Files);
        Assert.Equal(Path.Combine(directory, "NamedNamespace.cs"), outputWriter.Files.First());
    }
    ...
    public void TestEmptyKeyProviderBothPrefixes()
    {
        ...
        SharedTestFunctions.TestSamples(output, "EmptyKeyProviderBothPrefixes", PrefixPattern);
        Assert.Single(outputWriter.Files);
        Assert.Equal(Path.Combine(directory, "NamedNamespace.cs"), outputWriter.Files.First());
    }
    ...
    public void TestFullKeyProvider()
    {
        ...
        SharedTestFunctions.TestSamples(output, "FullKeyProvider", PrefixPattern);
        Assert.Single(outputWriter.Files);
        Assert.Equal(Path.Combine(directory, "NamedNamespace.cs"), outputWriter.Files.First());
    }
    ...
    public void TestFullKeyProviderGeneratorPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "FullKeyProviderGeneratorPrefix", PrefixPattern);
        Assert.Single(outputWriter.Files);
        Assert.Equal(Path.Combine(directory, "NamedNamespace.cs"), outputWriter.Files.First());
    }
    ...
    public void TestFullKeyProviderGeneratorConfigurationPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "FullKeyProviderGeneratorConfigurationPrefix", PrefixPattern);
        Assert.Single(outputWriter.Files);
        Assert.Equal(Path.Combine(directory, "NamedNamespace.cs"), outputWriter.Files.First());
    }
    ...
    public void TestFullKeyProviderBothPrefixes()
    {
        ...
        SharedTestFunctions.TestSamples(output, "FullKeyProviderBothPrefixes", PrefixPattern);
        Assert.Single(outputWriter.Files);
        Assert.Equal(Path.Combine(directory, "NamedNamespace.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateDefaultProviderGeneratorPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateDefaultProviderGeneratorPrefix", PrefixPattern);
    ...
    public void TestSeparateWithNamespaceProviderGeneratorPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateWithNamespaceProviderGeneratorPrefix", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "Generator", "Prefix", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateEmptyKeyProvider()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateEmptyKeyProvider", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
    ...
    public void TestSeparateEmptyKeyProviderGeneratorPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateEmptyKeyProviderGeneratorPrefix", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "NamedNamespace", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateEmptyKeyProviderGeneratorConfigurationPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateEmptyKeyProviderGeneratorConfigurationPrefix", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "NamedNamespace", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateEmptyKeyProviderBothPrefixes()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateEmptyKeyProviderBothPrefixes", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "NamedNamespace", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateFullKeyProvider()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateFullKeyProvider", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "NamedNamespace", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateFullKeyProviderGeneratorPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateFullKeyProviderGeneratorPrefix", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "NamedNamespace", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateFullKeyProviderGeneratorConfigurationPrefix()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateFullKeyProviderGeneratorConfigurationPrefix", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "NamedNamespace", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
    ...
    public void TestSeparateFullKeyProviderBothPrefixes()
    {
        ...
        SharedTestFunctions.TestSamples(output, "SeparateFullKeyProviderBothPrefixes", PrefixPattern);
        Assert.Equal(2, outputWriter.Files.Count());
        Assert.Equal(Path.Combine(directory, "NamedNamespace", "PurchaseOrderType.cs"), outputWriter.Files.First());
    }
}
